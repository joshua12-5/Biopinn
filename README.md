...
| `losses.py` | The five composite loss terms (data, physics/PDE-residual via autograd, Dirichlet BC, Neumann BC, IC). | training, evaluation (PDE-residual stats), ablation |
| `train.py` | Two-phase Adam → L-BFGS training engine: gradient clipping, StepLR, `w_phys` warmup ramp, NaN-recovery safeguard, checkpointing. | Colab notebook, `ablation.py` (baseline training) |
